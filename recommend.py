...
def recommend(vec:list[int],k: int,pivot_table: pd.DataFrame, avg_wage:pd.DataFrame) -> pd.DataFrame:
    """
        Takes a vector(attribute vector), avg_wage, pivot_tableand k(number of recommendations) and recommends k teams 
    """
...
if __name__ == "__main__":
    ...
    print(recommend(v,5,pd.read_csv("./final_data.csv")))